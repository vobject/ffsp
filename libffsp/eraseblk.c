...
static uint32_t find_empty_eraseblk(const struct ffsp *fs)
{
...
int ffsp_get_eraseblk_type(const struct ffsp *fs, int data_type, uint32_t mode)
{
...
int ffsp_find_writable_cluster(struct ffsp *fs, const int eb_type,
				uint32_t *eb_id, uint32_t *cl_id)
{
	...
	for (unsigned int eb = 1; eb < fs->neraseblocks; ++eb) {
		...
		if (cur_writeops < max_writeops) {
			...
			*cl_id = eb * fs->erasesize / fs->clustersize
				+ cur_writeops;
			return 0;
		}
	}
...
void ffsp_commit_write_operation(struct ffsp *fs, const int eb_type,
				const uint32_t eb_id, const be32_t ino_no)
{
...
int ffsp_write_meta_data(const struct ffsp *fs)
{
	...
	eb_usage_size = fs->neraseblocks * sizeof(struct ffsp_eraseblk);

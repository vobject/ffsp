		...
		struct fuse_file_info *fi)
{
	...
	FFSP_DEBUG("fuse_ffsp_readdir(path=%s, offset=%jd)", path, offset);
...
static int fuse_ffsp_truncate(const char *path, off_t length)
{
	int rc;
	struct ffsp_inode *ino;

	FFSP_DEBUG("fuse_ffsp_truncate(path=%s, length=%jd)", path, length);
...
{
	FFSP_DEBUG("fuse_ffsp_read(path=%s, count=%zu, offset=%jd)",
			path, count, offset);
	...
	return ffsp_read(&fs, get_inode(fi), buf, count, offset);
}

static int fuse_ffsp_write(const char *path, const char *buf, size_t count,
		off_t offset, struct fuse_file_info *fi)
{
	FFSP_DEBUG("fuse_ffsp_write(path=%s, count=%zu, offset=%jd)",
			path, count, offset);
	...
	return ffsp_write(&fs, get_inode(fi), buf, count, offset);
}
...
static int fuse_ffsp_mknod(const char *path, mode_t mode, dev_t device)
{
	...
	FFSP_DEBUG("fuse_ffsp_mknod(path=%s, mode=0x%x, device=%jd)",
			path, mode, device);
